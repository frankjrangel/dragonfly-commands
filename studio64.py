...
# This defines a configuration object with the name "studio64".
config            = Config("studio64")
config.cmd        = Section("Language section")


# This searches for a file with the same name as this file (studio64.py), but with
# the extension ".py" replaced by ".txt". In other words, it loads the
# configuration specified in the file studio64.txt
...
studio64_window_rule = MappingRule(
    name = "studio64_window",
    mapping = {
...
studio64_tabulator_rule = MappingRule(
    name = "studio64_tabulators",
    mapping = {
...
studio64_general_rule = MappingRule(
    name = "studio64_general",
    mapping = {
        "cancel": Key("escape,u"),
        },
    extras = [
...
studio64_navigation_rule = MappingRule(
    name = "studio64_navigation",
    mapping = {
...
studio64_exec_context = AppContext(executable="studio64")
...
ExModeGrammar = Grammar("ExMode grammar", context=studio64_context)
...
InsertModeBootstrap = Grammar("InsertMode bootstrap", context=studio64_context)
InsertModeBootstrap.add_rule(InsertModeEnabler())
InsertModeBootstrap.load()
InsertModeGrammar = Grammar("InsertMode grammar", context=studio64_context)
...
normalModeGrammar = Grammar("studio64", context=studio64_context)
normalModeGrammar.add_rule(NormalModeRepeatRule())
normalModeGrammar.add_rule(studio64_window_rule)
normalModeGrammar.add_rule(studio64_tabulator_rule)
normalModeGrammar.add_rule(studio64_general_rule)
normalModeGrammar.add_rule(studio64_navigation_rule)
